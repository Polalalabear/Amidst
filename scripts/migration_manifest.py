...
ROOT_ALIAS_RE = re.compile(r"^[a-z][a-z0-9-]{0,63}$")
...
class RootBinding:
    def __init__(self, alias: str, path: Path):
        if not ROOT_ALIAS_RE.fullmatch(alias):
            raise ValueError(f"Invalid root alias: {alias}")
        self.alias = alias
        self.path = path.resolve()


def root_binding(value: str | Path) -> RootBinding:
    text = str(value)
    alias, separator, raw_path = text.partition("=")
    if separator:
        if not raw_path:
            raise ValueError(f"Root binding requires a path: {text}")
        return RootBinding(alias, Path(raw_path))
    return RootBinding("repository", Path(text))
...
def parse_logical_entry(value: str) -> dict[str, str]:
    classification, separator, remainder = value.partition(":")
    alias, second_separator, path = remainder.partition(":")
    if (
        not separator
        or not second_separator
        or classification not in CLASSIFICATIONS
        or not ROOT_ALIAS_RE.fullmatch(alias)
    ):
        raise ValueError(
            "--logical-entry must be CLASSIFICATION:root-alias:relative/path"
        )
    portable_path(path)
    return {"classification": classification, "root_alias": alias, "path": path}


def load_inventory(
    path: Path | None,
    command_entries: list[str],
    logical_command_entries: list[str] | None = None,
) -> list[dict[str, str]]:
    ...
    if path is not None:
        ...
        for entry in value["entries"]:
            ...
            if entry.get("root_alias") is not None:
                record["root_alias"] = str(entry.get("root_alias"))
            entries.append(record)
    entries.extend(parse_entry(value) for value in command_entries)
    entries.extend(
        parse_logical_entry(value) for value in (logical_command_entries or [])
    )
    if not entries:
    ...
    for entry in entries:
        ...
        if "root_alias" in entry and not ROOT_ALIAS_RE.fullmatch(entry["root_alias"]):
            raise ValueError(f"Invalid root alias for {entry['path']}")
    return entries
    ...
    for root in roots:
        ...
        if candidate.exists():
            ...
            if (
                ...
                raise ValueError(
                    f"Migration path escapes configured root: {relative.as_posix()}"
                )
            return candidate
...
) -> tuple[Path, RootBinding]:
    candidates = [
        binding
        for binding in bindings
        if requested_alias is None or binding.alias == requested_alias
    ]
    if requested_alias is not None and not candidates:
        raise FileNotFoundError(f"Configured root alias is missing: {requested_alias}")
    for binding in candidates:
        try:
            return find_source(relative, [binding.path]), binding
...
def expand_entries(
    entries: Iterable[dict[str, str]], roots: list[Path | RootBinding]
) -> list[dict[str, Any]]:
    bindings = [
        value if isinstance(value, RootBinding) else RootBinding("repository", value)
        for value in roots
    ]
    ...
    for requested in entries:
        ...
        source, binding = find_bound_source(
            relative, bindings, requested.get("root_alias")
        )
        candidates = [source]
        if source.is_dir():
            candidates = sorted(
        ...
        for candidate in candidates:
            ...
            logical_uri = f"asset://{binding.alias}/{logical_text}"
            record = {
                "path": logical_text,
                "logical_uri": logical_uri,
                "root_alias": binding.alias,
                "relative_path": logical_text,
                "classification": requested["classification"],
                "size_bytes": candidate.stat().st_size,
                "sha256": sha256_file(candidate),
            }
            previous = expanded.get(logical_uri)
            if previous is not None and previous != record:
                raise ValueError(f"Conflicting inventory entries: {logical_uri}")
            expanded[logical_uri] = record
...
def create_manifest(options: argparse.Namespace) -> int:
    roots = [root_binding(value) for value in options.source_root]
    named_aliases = [binding.alias for binding in roots if binding.alias != "repository"]
    if len(set(named_aliases)) != len(named_aliases):
        raise ValueError("Every named --source-root alias must be unique")
    if any(not binding.path.is_dir() for binding in roots):
        raise ValueError("Every --source-root must be an existing directory")
    inventory = load_inventory(
        options.inventory, options.entry, getattr(options, "logical_entry", [])
    )
    ...
    manifest = {
        ...
        "path_contract": {
            "base": "configured logical roots",
            "separator": "/",
            "absolute_paths_serialized": False,
            "physical_roots_serialized": False,
            "parent_segments_allowed": False,
            "git_metadata_included": False,
            "logical_uri_scheme": "asset://<root-alias>/<relative-path>",
            "nested_symlinks_allowed": False,
        },
        "repository": git_record((options.git_root or Path.cwd()).resolve()),
        "source_environment": {
        ...
        json.dumps(
            {
                ...
                "physical_output_path_serialized": False,
            },
            sort_keys=True,
        )
    )
...
def verify_manifest(options: argparse.Namespace) -> int:
    ...
    targets = [root_binding(value) for value in options.target_root]
    if any(not target.path.is_dir() for target in targets):
    ...
    for record in entries:
        ...
        root_alias = record.get("root_alias")
        if root_alias is not None and not ROOT_ALIAS_RE.fullmatch(str(root_alias)):
            raise ValueError(f"Invalid migration root alias: {root_alias}")
        expected_uri = (
            f"asset://{root_alias}/{relative.as_posix()}"
            if root_alias is not None
            else None
        )
        if record.get("logical_uri") is not None and record.get("logical_uri") != expected_uri:
            raise ValueError("Migration logical URI does not match its root and path")
        record_key = str(record.get("logical_uri") or relative.as_posix())
        if record_key in seen:
            raise ValueError(f"Duplicate migration manifest path: {record_key}")
        seen.add(record_key)
        try:
            candidate, _ = find_bound_source(
                relative,
                targets,
                str(root_alias) if root_alias is not None else None,
            )
        except FileNotFoundError:
            reason = "root_unavailable" if root_alias is not None else "missing"
            failures.append({"path": record_key, "reason": reason})
            continue
        if not candidate.is_file():
            failures.append({"path": record_key, "reason": "not_a_file"})
            continue
        if candidate.is_symlink():
            failures.append({"path": record_key, "reason": "symlink"})
            continue
...
def parser() -> argparse.ArgumentParser:
    ...
    create.add_argument("--logical-entry", action="append", default=[])
